...
for exp_dir in "$@"; do
    ...
    if [ ! -f $exp_dir/dev.word_accuracy ] || [ $exp_dir -nt $exp_dir/dev.word_accuracy ]; then
        ...
        covered=$(dirname $dev_file)/${lang}-${track}-covered
        python $INFERENCE_SRC -e $exp_dir -t $covered > $exp_dir/dev.out
        paste $exp_dir/dev.out $dev_file | cut -f1,4,6 | grep "_" | awk 'BEGIN{FS="\t"}{if($1==$2)c++;s++}END{print c/s}' > $exp_dir/dev.word_accuracy
